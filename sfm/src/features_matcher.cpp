...
void FeatureMatcher::exhaustiveMatching()
{
  for( int i = 0; i < images_names_.size() - 1; i++ )
  {
    for( int j = i + 1; j < images_names_.size(); j++ )
    {
      ...
      std::vector<int> inlier_mask_H;
      cv::Mat inlier_mask_E;

      for (int f = 0; f < matches.size(); f++)
      {
        src_pts.push_back(features_[i][matches[f].queryIdx].pt);
        dst_pts.push_back(features_[j][matches[f].trainIdx].pt);
      }

      cv::findHomography(src_pts, dst_pts, cv::RANSAC, 3, inlier_mask_H);  
      cv::findEssentialMat(src_pts, dst_pts, new_intrinsics_matrix_, cv::RANSAC, 0.999, 1.0, inlier_mask_E);

      std::vector<int> inlier_mask_E_ = (std::vector<int>)inlier_mask_E;

      std::vector<int> best_model;
      int num_inliers_H = cv::countNonZero(inlier_mask_H);
      int num_inliers_E = cv::countNonZero(inlier_mask_E_);

      if (num_inliers_H > num_inliers_E && num_inliers_H > 300)
      {
        best_model = inlier_mask_H;
      }
      else if (num_inliers_E > num_inliers_H && num_inliers_E > 300)
      {
        best_model = inlier_mask_E_;
      }
