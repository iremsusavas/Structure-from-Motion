...
struct ReprojectionError
{
  ...
  {
    return (new ceres::AutoDiffCostFunction<ReprojectionError, 2, 6, 3>( //num residuals, size camera block, size point block
...
void BasicSfM::solve()
{
  ...
  while( !seed_found )
  {
    ...
    cv::Mat H = cv::findHomography(points0, points1, cv::RANSAC, 3, inlier_mask_H); 
    ...
    if (num_inliers_E > num_inliers_H)
    {
      ...
      for (int k = 0; k < inlier_mask_E_.size(); k++)
      {
        if (inlier_mask_E_[k] == 1)
        {
